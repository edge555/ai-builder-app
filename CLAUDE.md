...
1. **Incremental JSON Parser (Task 5.2)**: O(n²) → O(n) optimization
   - Single-pass character scanning eliminates repeated `indexOf()` calls
   - 10MB with 100 files: ~23ms (target: < 100ms) ✅
   - Throughput: 403 MB/s
   - File: `backend/lib/utils/incremental-json-parser.ts`

2. **Dependency Graph Caching (Task 5.3)**: Content-based caching
   - Cache hits: 152x speedup (10.56ms → 0.07ms)
   - Pre-computed path lookup for O(1) import resolution
   - SHA-256 hash of file paths + content hashes as cache key
   - File: `backend/lib/analysis/dependency-graph.ts`

3. **Split Context Pattern**: Reduce re-renders across the app
   - Implemented for: GenerationContext, PreviewErrorContext, ChatMessagesContext, VersionContext
   - Components subscribe only to state OR actions, not both
   - Prevents cascading re-renders through 7+ nested providers

4. **React.memo Optimization**: Expensive components wrapped with custom comparators
   - PreviewPanel (Sandpack), ChatInterface, CodeEditorView (Monaco), ProjectGallery
   - Deep-compare critical props, use refs to cache transformed data
   - Expected 50%+ reduction in unnecessary renders

5. **List Virtualization**: `@tanstack/react-virtual` for large datasets
   - ProjectGallery (> 20 projects), ChatInterface (> 20 messages), FileTreeSidebar (> 50 files)
   - Only visible items + buffer rendered in DOM
   - Smooth scrolling with 500+ items

6. **Error Boundaries**: Graceful degradation for component failures
   - ComponentErrorBoundary with retry functionality
   - PreviewErrorBoundary with auto-repair integration
   - Prevents app crashes from lazy-loaded component failures

7. **Debounced Search**: `useDeferredValue` prevents UI blocking
...
1. **Task 5.1**: Move Prettier formatting to worker pool
   - Current: Synchronous Prettier blocks event loop
   - Target: Worker threads for non-blocking formatting

2. **Task 5.4**: Split large components (partial completion)
   - AppLayout: 530 lines (target: < 200 lines)
   - ChatInterface: 571 lines (target: < 200 lines)
   - PreviewPanel: 393 lines (target: < 200 lines)
   - ErrorOverlay extracted from AppLayout ✅

3. **Task 5.5**: Add input validation & size limits to API routes
   - Need: Prompt length limits, file count/size limits, path traversal protection

4. **Task 5.6**: Use request headers for Gemini API key
   - Current: API key in URL query parameter
   - Target: `x-goog-api-key` header

5. **Task 5.7**: Add comprehensive ESLint rules
   - Need: `no-console`, `no-unused-vars`, import ordering, security rules

6. **Task 5.8**: Improve streaming error recovery
   - Need: File status field, stream-end event, formatting failure warnings
