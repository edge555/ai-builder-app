...
// Shared mock functions returned by every WorkerPool instance
...
vi.mock('../worker-pool', () => ({
  WorkerPool: vi.fn().mockImplementation(function() {
    return { runTask: mockRunTask, terminate: mockTerminate };
  }),
}));
...
describe('processFile', () => {
  ...
  describe('path sanitization', () => {
    it('should remove spaces from file paths', async () => {
      mockRunTask.mockResolvedValue('formatted content');
  ...
  describe('content normalization', () => {
    it('should convert \\n to actual newlines', async () => {
      mockRunTask.mockImplementation(async ({ content }: { content: string }) => content);
    ...
    it('should fix literal CRLF in strings', async () => {
      ...
      const result = await processFile('test.ts', "split('\\r\\n')");
      // STEP 1 converts \n (backslash+n) to actual newline; CRLF case leaves \r then newline
      expect(result.content).toBe("split('\\r\n')");
    });
    ...
    it('should handle double-escaped sequences from JSON', async () => {
      ...
      // Input: line1\\nline2 (two backslashes + n). STEP 1 converts the trailing \n → newline,
      // leaving the first backslash intact. Result: line1\ + newline + line2.
      const result = await processFile('test.ts', 'line1\\\\nline2');
      expect(result.content).toBe('line1\\\nline2');
    });
    ...
    it('should handle double-escaped tabs', async () => {
      ...
      // Input: line1\\tline2. STEP 1 converts \t → tab, leaving first backslash + tab.
      const result = await processFile('test.ts', 'line1\\\\tline2');
      expect(result.content).toBe('line1\\\tline2');
    });
  });
  ...
  describe('prettier formatting', () => {
    ...
    it('should handle prettier errors gracefully', async () => {
      mockRunTask.mockRejectedValue(new Error('Prettier error'));
    ...
    it('should return warning when formatting fails', async () => {
      mockRunTask.mockRejectedValue(new Error('Syntax error'));
  ...
  describe('complex scenarios', () => {
    it('should handle complete processing pipeline', async () => {
      ...
      expect(result.path).toBe('frontend/src/App.tsx');
      // STEP 1 converts \n (backslash+n) inside the string literal to an actual newline
    ...
    it('should handle file with escaped sequences', async () => {
      ...
      // Input has double-escaped \\n and \\t. STEP 1 converts trailing \n→newline and \t→tab,
      // leaving first backslash before each.
      const content = 'line1\\\\nline2\\\\tline3';
      const result = await processFile('test.ts', content);
