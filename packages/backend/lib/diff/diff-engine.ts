...
import type { FileDiff, DiffHunk, DiffChange, ChangeSummary, ProjectState } from '@ai-app-builder/shared';
 ...
 */
function collapseIdenticalDiffLines(lines: DiffLine[]): DiffLine[] {
  const result: DiffLine[] = [];
  let i = 0;
 ...
 */
function groupIntoHunks(diffLines: DiffLine[], contextLines: number = DIFF_CONTEXT_LINES): DiffHunk[] {
 ...
 */
function hasRealChanges(hunks: DiffHunk[]): boolean {
...
export function computeDiffs(
  ...
  for (const filePath of allPaths) {
    ...
    // If normalized content is equal, no diff needed
  }
  
  // Sort diffs by file path for consistent ordering
  const sortedDiffs = diffs.sort((a, b) => a.filePath.localeCompare(b.filePath));
